...
for tp = {'CS' 'US' 'SW' 'NS'}
    ...
    toc;
    save([spath 'mat_thrshd_read_' tp{1} '.mat'],'threshold_img');
    save([spath 'mat_distri_read_' tp{1} '.mat'],'distribution_img');
    nii_out = make_nii(threshold_img, [3 3 3]);
    save_nii(nii_out,[spath 'thrshd_read' tp{1}, '.nii']);    

    nii_out = make_nii(distribution_img, [3 3 3]);
    save_nii(nii_out,[spath 'distr_read' tp{1}, '.nii']);    
end
